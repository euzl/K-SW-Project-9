#ifndef __BLUETOOTHSERVER__
#define __BLUETOOTHSERVER__
...
void write_server(int client, char *message) {
	...
	strcat(messageArray, messageArr);
...
int startBluetoothServer(double* _loc_val)
{
    int client = init_server();
	//_loc_val[4]={0, 0, 0, 0};			//save location values as double type (1: target latitude, 2: target longitude, 3: empty, 4: empty)
	int loc_val_idx=0;					//index fro loc_value array
    while(1)
    {
		...
		//sscanf(recv_message, "%lf", loc_value + loc_val_idx);
		_loc_val[loc_val_idx] = atof(recv_message);
		loc_val_idx ++;
	...
	if (_loc_val[0] != 0 && _loc_val[1] != 0 && _loc_val[2] != 0 && _loc_val[3] != 0) {
		...
		printf("recieved the target and mortar location fialed!\n");
		printf("%8.6lf %8.6lf %8.6lf %8.6lf", _loc_val[0], _loc_val[1], _loc_val[2], _loc_val[3]);
		return 0;
