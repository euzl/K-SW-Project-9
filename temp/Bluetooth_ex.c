...
int bluetoothServerOpen()
{
	int client = init_server();
	double loc_value[4] = { 0, 0, 0, 0 };			//save location values as double type (1: target latitude, 2: target longitude, 3: empty, 4: empty)
	...
	while (1)
	{
		...
		loc_value[loc_val_idx] = atof(recv_message);
		loc_val_idx++;
		printf("recv_message : %s\n", recv_message);
	}
	printf("sum: %lf", loc_value[0] + loc_value[1]);
	if (loc_value[0] != 0 && loc_value != 0) {
