...
public class BluetoothSerialClient {
    ...
    protected BluetoothSerialClient() {
...
//        while(!mIsConnection.get()){
//
//        }
        if(!mIsConnection.get()) {
            Log.println(Log.ERROR, "bluetooth connect", "mlsConnection failed");
            return false;
        }
        mWriteExecutor.execute(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    Log.println(Log.ERROR, "bluetooth connect", "mOutputStream write failed");
                    close();
                    e.printStackTrace();
                    mBluetoothStreamingHandler.onError(e);
                }
    ...
    public abstract static class BluetoothStreamingHandler {
        ...
        public final boolean write(byte[] buffer) {
            ...
            if(btSet != null) {
                Log.println(Log.ERROR, "bluetooth connect", "bluetooth connection can get instance ");
                return btSet.write(buffer);

            }Log.println(Log.ERROR, "bluetooth connect", "bluetooth connection cannot get instance");
            return false;
        }
    }
